...
class UserService {
  ...
  async getCurrentUserProfile(): Promise<UserProfile> {
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://192.168.1.4:3000'}/api/users/profile`, {
        method: 'GET',
  ...
  async getUserStats(): Promise<UserStats> {
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://192.168.1.4:3000'}/api/users/stats`, {
        method: 'GET',
        headers: this.getAuthHeaders(),
      });
  ...
  async updateStatus(status: 'online' | 'offline' | 'in-game'): Promise<void> {
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://192.168.1.4:3000'}/api/users/status`, {
        method: 'PUT',
        headers: {
          ...this.getAuthHeaders(),
  ...
  async getUserById(userId: string): Promise<UserProfile> {
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://192.168.1.4:3000'}/api/users/${userId}`, {
        method: 'GET',
        headers: this.getAuthHeaders(),
      });
