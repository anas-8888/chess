...
class AuthService {
  ...
  public async logout(): Promise<void> {
    ...
    if (this.token) {
      try {
        const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://192.168.1.6:3000'}/api/auth/logout`, {
          method: 'POST',
          headers: {
  ...
  public async validateToken(): Promise<boolean> {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://192.168.1.6:3000'}/api/auth/validate`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${this.token}`
