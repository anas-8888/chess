...
const Friends = () => {
  ...
  const setupSocketListeners = () => {
    const socket = io(`${import.meta.env.VITE_API_URL || 'http://192.168.1.4:3000'}/friends`, {
      auth: {
  ...
  const sendGameInviteToBackend = async () => {
    ...
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://192.168.1.4:3000'}/api/invites/game`, {
        method: 'POST',
        headers: {
